# Reference: Marc Höftmann , DDQN-Atari-PyTorch
# GitHub. https://github.com/Hauf3n/DDQN-Atari-PyTorch
################################

import torch
import gym
from Atari_Wrapper import Atari_Wrapper
import time
import numpy as np

from Agent import DQN_Agent, KQN_Agent

from datetime import datetime
from config import ARGS as args

np.bool8 = np.bool_
dtype = torch.float


def play(env):
    env_name = env
    num_stacked_frames = args.stacked_frames
   
    raw_env = gym.make(env_name) #, render_mode = 'human')
    env = Atari_Wrapper(raw_env, env_name, num_stacked_frames)

    # Create a new model with the same architecture as the saved model
    # This is the key step - we need to match the architecture that was used during training
    from Network import DQN, KQN
    
    # Look at the saved model structure
    saved_model = torch.load(f'Student/{env_name.replace("-","_")}.pt', map_location=torch.device('cpu'))
        
    # Create and load the agent with matching architecture
    agent = KQN_Agent(num_stacked_frames, env.action_space.n, 0)
    
    # Try loading with strict=False to ignore mismatches
    agent.load_state_dict(saved_model, strict=False)
    agent = agent.cuda()
    agent.eval()
    agent.set_epsilon(0)

    steps = 50000
    ob = env.reset()
    imgs = []
    ep_reward = 0
    for _ in range(steps):
        ob1 = torch.tensor(ob, dtype=dtype)
        ob1 = ob1.unsqueeze(0).cuda()
        action = agent.e_greedy(ob1 / 255)
        action = action.detach().cpu().numpy()
        # print(action)
        ob, reward, done, info, _ = env.step(action)
        ep_reward += reward
        # time.sleep(0.016)        
        if done:
            ob = env.reset()
            print(info)
            print(f"Episode finished. Reward: ", ep_reward)
            ep_reward = 0
        imgs.append(ob)
        
    env.close()
    return imgs

def main(env):
    _ = play(env)

if __name__ == '__main__':
    main(env='PongNoFrameskip-v4')